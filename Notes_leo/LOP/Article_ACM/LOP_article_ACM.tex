...
The three model we have used in our work derive from the Graphical Probabilistic Model (GPM, \cite{fischer2014training}) theory. The RBM, CRBM and FGCRBM (\cite{taylor2009composable}) are then presented by increasing level of complexity, each model adding on the previously defined model a new element.
...
To model time series, if we consider that the visible units $\textbf{v}(t)$ represent the current time frame, those context units can be defined as the concatenation of the N last time frames. We call this vector $\bm{x}(t) = \left( v_{1}^{(t-N)} , ... , v_{m}^{(t-N)}, ... , v_{1}^{(t)} ... , v_{m}^{(t)} \right)$, where N denotes the order of the model.
The energy function of the Conditional RBM is given by (\prettyref{eq:energy_CRBM})
\begin{equation}
\label{eq:energy_CRBM}
E(\textbf{v}(t),\textbf{h}(t)|\textbf{x}(t)) = - \sum_{i} \hat{a}_{i}(t)v_{i}(t) - \sum_{ij}W_{ij}v_{i}(t)h_{j}(t) - \sum_{j} \hat{b}_{j}(t)h_{j}(t)
\end{equation}
where the biases are defined by $\hat{a}_{i}(t) = a_{i} + \sum_{k}A_{ki}x_{k}(t)$ and $\hat{b}_{j} = b_{j} + \sum_{k}B_{kj}x_{k}(t)$.

The updates rules are then given by
\begin{align}
\Delta W_{ij} &= <v_{i}h_{j} >_{data} - <v_{i}h_{j} >_{model}\\
\Delta b_{i}^{(v)} &= <v_{i}>_{data} - <v_{i}>_{model}\\
\Delta b_{j}^{(h)} &= <h_{j} >_{data} - <h_{j} >_{model}\\
...
\begin{equation}
E(\textbf{v}(t),\textbf{h}(t)|\textbf{x}(t),\textbf{z}(t)) = -\sum_{f}\sum_{ijl} W_{if}^{v} W_{jf}^{h} W_{lf}^{z} v_{i}(t) h_{j}(t) z_{l}(t) 
